...
| Version | Changes |
|---------|---------|
| **1.22.1** | **WebSocket stability fix: quiet systems no longer disconnect every 3 minutes.** Removed the 180s timeout around each WebSocket `receive()` call. On installations with sparse WebSocket traffic the timeout expired on perfectly healthy connections — aiohttp answers server pings internally, so a quiet connection legitimately produces no frames — causing a spurious ERROR log, a full teardown and reconnect every 3 minutes, a flapping "WebSocket disconnected" repair notification, and in-flight commands failing during each flap. Dead connections are still detected by the aiohttp heartbeat (30s ping), now pinned by a regression test. Also: `@types/node` realigned to the Node 24 floor; dead `[tool.pytest.ini_options]` block removed from pyproject.toml. Thanks to Gernot Reisinger (@Hollerberg) for the diagnosis and fix (#6, merged via #10). Full suite: 1276 tests. |
| **1.22.0** | **Post-command quiesce redesign + coordinator/light fixes + MCP refactor + runtime floor bump (Python 3.14 / Node 24)** — Major UX fix: command → optimistic state → HTTP recheck pattern replaced with a 5s quiesce window that any incoming WS event cancels, eliminating the post-command bounce-back where the UI briefly flipped back to the stale "on" state after turning a light off. Single `POST_COMMAND_QUIESCE_PERIOD = 5s` replaces the old 2.5s/1.0s settling constants across lights, switches, bathroom radiators, covers, climate, and selects. Climate gained a self-healing recheck fallback. The radiator's 3s post-toggle verify timer is removed (subsumed by the new mechanism). HTTP load on the Evon controller drops significantly since most commands no longer trigger an immediate poll. **Runtime floors raised (breaking):** minimum HA version bumped to 2026.5.0, which itself requires Python 3.14.2+. The integration now targets Python 3.14 and the bundled MCP server requires Node.js 24 LTS. Note that the source contains 3.14-only syntax, so an older interpreter fails with a `SyntaxError` rather than a clean version warning — this affects nobody who can run HA 2026.x, since that already requires 3.14.2. This also fixes a silent test gap: with an open version floor, the test harness resolved to HA 2025.1.4 under Python 3.12, so the suite had never exercised a supported HA release. CI now runs Python 3.14 against HA 2026.8.0, and the MCP server's Node version is aligned across `engines`, CI, and `@types/node`. **Also:** coordinator now uses `async_update_listeners()` for WS events so real-time updates don't reset the polling timer; dimmable lights report `brightness=0` (not stale/None) when off; MCP server replaces `validateConfig()` void-return with `requireEvonHost()`; npm dependencies updated and remaining audit vulnerabilities resolved; codecov-action bumped to v6; deploy script uses a dedicated SSH key. **WS/poll race fix:** WS updates arriving during a slow in-flight HTTP poll are now preserved instead of being overwritten by the poll's stale snapshot, so a light toggled on no longer reverts to "off" for ~20s until the next poll. **Pre-release review fixes:** camera recordings are served through an authenticated endpoint instead of an open static path (footage was previously downloadable by anyone who could reach the HA port); energy-statistics import respects its 1-hour rate limit again (it was re-importing every poll); WebSocket reconnect applies backoff when the post-connect handshake fails (was a tight retry loop); repair "Fix" flows moved to `repairs.py` so Home Assistant can discover them; physical-button event entities no longer drop a press after a safety-net poll resets the internal counter; and stuck optimistic state for light color-temperature and cover toggle-stop is now cleared by the 30s backstop. **Reliability & hardening (review follow-ups):** config-entry unload no longer leaves a half-torn-down entry when platform unload fails; a wrong password at startup now opens the re-authentication dialog, while a transient login rate-limit retries instead of prompting for credentials; stale-entity cleanup no longer deletes entities over a transient fetch error or mis-truncates instance IDs containing underscores; the diagnostics download redacts the host / Engine ID in the config-entry title; WebSocket resubscribe retries after a dropped subscription, an unsubscribe issued while offline is honoured, and malformed close/error frames log the real reason; camera-recording finalize is guarded against a double-encode race; plus instance-ID and image-path input-validation hardening. The post-command recheck now keys off genuine WebSocket activity (a slow in-flight poll can no longer cancel the safety net), blind stop / toggle-stop fetch their resting position promptly on WebSocket-less setups, and re-entrant coordinator refreshes are guarded. Dead API-client methods removed and magic numbers replaced with named constants. **Second review pass (RV2):** saving integration options no longer pops a phantom "WebSocket disconnected" repair (intentional teardown was indistinguishable from a failure); a WS confirmation racing the command response now counts as liveness, so relays no longer trigger a redundant full poll 5s after every toggle; smart-meter WS chatter no longer spawns a no-op statistics task every few seconds; bulk services share the exact instance-ID validation of the API layer; concurrent first refreshes are serialized. Full suite: 1277 tests. |
| **1.21.0** | **HA 2026.3 compatibility, HACS hardening, async & security fixes** - Minimum HA version bumped to 2026.3.0. MCP server now covers all 4 HTTP light class types (`SmartCOM.Light.Light`, `LightDim`, `DynamicRGBWLight`, `LightGroup`). SSL context creation moved off the event loop. API client switched to HA-managed session callback. Re-auth failures now trigger HA's reauth UI. Repair notifications scoped per config entry and auto-resolved. WebSocket disconnect raises HA Repair notification. `py.typed` marker added. 3 CodeQL findings resolved. Patched flatted prototype pollution vulnerability (high severity). |
| **1.20.0** | **Relay outputs as light entities (breaking change)** - Evon relay modules (`SmartCOM.Light.Light` / L1144, L1244) are now exposed as on/off light entities (`ColorMode.ONOFF`) instead of switches. Old switch entities are automatically removed with an HA Repair notification. Code review: 12 fixes including WS race condition, brightness rounding, settling periods for climate/cover, camera auth retry, constants cleanup, and non-dimmable dropdown filtering. |
...
| **1.18.0** | **Auth retry storm fix (Issue #2), WebSocket diagnostics, doorbell event entity, 8 new translations.** Fixed auth retry storm regression causing 700+ API requests/min on network errors: login backoff on network errors, safe re-auth wrapping to prevent token=None cascades, WS receive timeout relaxed from 90s to 180s (timeout removed entirely in v1.22.1). Added WebSocket diagnostic sensors (connection status with 7 attributes, response latency with long-term statistics). Added doorbell event entity for 2N intercoms (native HA EventEntity with ring detection). Added 8 translations: French, Italian, Slovenian, Spanish, Portuguese, Polish, Czech, Slovak. CI: bumped GitHub Actions, test dep cleanup, ruff format fixes. |
