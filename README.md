...
| Version | Changes |
|---------|---------|
| **1.17.1** | **Code quality audit** - Deep analysis and hardening across the integration. Fixed 5 bugs (light brightness rounding, climate cooling temp range, diagnostics crash, select entity errors, device trigger safety). Hardened WebSocket client (periodic stale request cleanup, error logging with stack traces, fire-and-forget error handling, sequenceId validation). Improved coordinator data consistency and input validation. Added 260+ new tests (820 total). |
