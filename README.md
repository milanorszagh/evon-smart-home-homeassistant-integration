...
**How it works:**
- **State updates**: WebSocket subscribes to device changes - when a light is turned on via wall switch, HA updates immediately
- **Device control**: Commands (light on/off, blind position, climate temperature) are sent via WebSocket for instant response
