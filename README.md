...
| Version | Changes |
|---------|---------|
| **1.22.0** | **Coordinator & light fixes, MCP refactor, dependency hygiene** - Coordinator now uses `async_update_listeners()` for WS events so real-time updates don't reset the polling timer. Dimmable lights report `brightness=0` (not stale/None) when off. Light and switch commands always refresh the coordinator after issuing a command. MCP server replaces `validateConfig()` void-return pattern with `requireEvonHost()` that returns the validated host, eliminating non-null assertions. npm dependencies updated and 5 audit vulnerabilities resolved. Deploy script now uses a dedicated SSH key. Documentation aligned with current code. |
