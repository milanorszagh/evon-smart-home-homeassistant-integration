...
| RV-D6 | Cleanup | `__init__.py` (`_async_cleanup_stale_entities`) | Cleanup ran on every setup gated only on `last_update_success`, which stays True even when `_safe_get_instance` silently drops individual instances on a transient fetch error — those entities were then removed as "stale", losing registry customizations. **Fixed:** the coordinator records `_last_poll_had_partial_failures` (any instance dropped by a fetch error); setup skips the cleanup when it's set (and logs why), so a transient miss no longer deletes entities. Cleanup still runs on the next clean setup. | Fixed |
