...
### Note #4: Switch (Relay) Control

Relay switches (`Base.bSwitch`) use `SwitchOn`/`SwitchOff` via WebSocket CallMethod, same as lights. HTTP fallback translates to `AmznTurnOn`/`AmznTurnOff`.

**Important naming distinction:** `SmartCOM.Switch` is NOT a relay — it's the Evon class for physical wall buttons (Tasters), exposed as event entities. Relay outputs use class `Base.bSwitch` (or `SmartCOM.Light.Light`).
```
// WebSocket (preferred):
...
Tests are in the `tests/` directory (1164 tests):
